...
set resourceFiles {"resSpecs/cpu_basic.tcl" "resSpecs/cpu_basic_low_power.tcl" "resSpecs/cpu_basic2.tcl" "resSpecs/cpu_commodity_4c.tcl" "resSpecs/cpu_commodity_2c.tcl" "resSpecs/cpu_high_end_8c.tcl" "resSpecs/cpu_micro_4c.tcl" "resSpecs/mem_basic2GB.tcl" "resSpecs/mem_basic4GB.tcl" "resSpecs/mem_basic6GB.tcl" "resSpecs/mem_basic8GB.tcl" "resSpecs/mem_basic10GB.tcl" "resSpecs/mem_basic32GB.tcl" "resSpecs/disk_basic.tcl" "resSpecs/disk_500GB.tcl" "resSpecs/disk_1000GB.tcl" "resSpecs/eth_basic.tcl"}
#Used virtual resource library:
set virtualResourceFiles {"resSpecs/virtual/vcpu_basic.tcl" "resSpecs/virtual/vcpu_basic_4core.tcl" "resSpecs/virtual/vcpu_basic_2core.tcl" "resSpecs/virtual/vmem_basic1GB.tcl" "resSpecs/virtual/vmem_basic2GB.tcl"  "resSpecs/virtual/vmem_basic4GB.tcl" "resSpecs/virtual/vmem_basic6GB.tcl" "resSpecs/virtual/vmem_basic8GB.tcl" "resSpecs/virtual/vdisk_basic.tcl" "resSpecs/virtual/vdisk_basic100GB.tcl" "resSpecs/virtual/veth_basic.tcl" "resSpecs/virtual/vdisk_basic10GB.tcl"  "resSpecs/virtual/veth_basic100MB.tcl"}
...
#	}
	...
	set DCResCPU($k) [new CPU]
   	...
   	#Computing	
		$DCenter configure-resource $DCResCPU($k) "Commodity processor 2 cores"
#		#Memory
		set DCResMem($k) [new DcResource]
		$DCenter configure-resource $DCResMem($k) "Nominal 4GB memory"
		$hosts_($k) add-resource $DCResMem($k)
	#Storage
		set DCResStor($k) [new DcResource]
		$DCenter configure-resource $DCResStor($k) "Nominal 1000GB disk"
...
for {set k 0} {$k < $top(NServers)} {incr k} {
	...
	$DCenter configure-vresource $VDCResCPU($k) "Nominal vcpu 1 core"
	$vms_($k) add-resource $VDCResCPU($k)
	
	#VMem
	set VDCResMem($k) [new DcResource]
	$DCenter configure-vresource $VDCResMem($k) "Nominal 4GB virtual memory"
	$vms_($k) add-resource $VDCResMem($k)
	
	#VDisk
	set VDCResStor($k) [new DcResource]
	$DCenter configure-vresource $VDCResStor($k) "Nominal 10GB virtual disk"
	$vms_($k) add-resource $VDCResStor($k)
	
	set VDCResNet($k) [new NIC]
	$DCenter configure-vresource $VDCResNet($k) "Nominal 100MB virtual NIC"
