...
set dc(target_load) 1.00		; # Fixed value or set as parameter: 
...
set vm(eDVFS_enabled) 	      1; # Analogous to serv(eDVFS_enabled). (VM uses the power of the hosting, so its internal scheduling behavior is important.)
...
switch $task(type) {
  default {						;# HPC - computationally intensive
    set task(mips)		9000000			;# [MIPS] 10^6 Million instructions
    set task(memory)		[lindex $argv 2]	;# [Byte] of required used RAM
    set task(storage)		0			;# [Byte] of required disk space
    set task(size)		5000			;# [Byte] 8500 B of task input size
...
set task(outputsize)	30000				;# Size of output on task completion
